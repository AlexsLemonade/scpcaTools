...
processing_info <- data.frame(
  ...
  "Index" = metadata_list$reference_index,
  "Filtering Method" = metadata_list$af_permit_type,
  "Resolution" = metadata_list$af_resolution, 
  "Unspliced transcripts included" = metadata_list$transcript_type
) %>% 
  dplyr::mutate(Unspliced.transcripts.included = dplyr::case_when(Unspliced.transcripts.included == "spliced" ~ FALSE,
                                 Unspliced.transcripts.included == "unspliced" ~ TRUE)) %>%
  t() %>%
...
sample_information <- data.frame(
  ...
  "Version 10X" = metadata_list$version_10X,
  "Number of Reads" = metadata_list$total_reads,
