...
The droplets that are expected to contain cells were identified with [`DropletUtils::emptyDropsCellRanger()`](https://bioconductor.org/packages/release/bioc/html/DropletUtils.html), which uses both the total UMI counts and expressed gene content ([Lun  _et al._ 2019](https://doi.org/10.1186/s13059-019-1662-y)).
...
if(has_processed){
  ...
  if(has_var_genes){
    max_gene <- processed_meta$highly_variable_genes[1]
    var_gene_exp <- counts(processed_sce[max_gene,]) %>%
      as.matrix() %>%
      t() %>%
      as.data.frame() %>%
    ...
    coldata_df <- colData(processed_sce) %>%
      as.data.frame() %>%
      tibble::rownames_to_column("barcode") %>%
      dplyr::left_join(var_gene_exp, by = "barcode")
    
    colData(processed_sce) <- DataFrame(coldata_df, row.names = coldata_df$barcode)
  }
